...
def filter_cols(dictdataframe):
    """
    Filtering metafile (e.g. meta_plate.tsv) by column "plate_id" in
    order to split one big pd.Dataframe
    into smaller ones, filtered by plate number (e.g plate1, plate2, ...)

    Args:
        dictdataframe (pd.DataFrame):   Dataframe, added by inserting path,
                                        Original Dataframe from metafile

    Raises:
        KeyError:   Searching for "plate_id" in Dataframe Column names.
                    Raises KeyError if "plate_id" is not found or
                    missing, prompts User to create Column names "plate_id"

    Returns:
        numbered_dict (Dictionary): Dictionary containing all of the
                                    split plates. Plate name gets replaced
                                    by index numbers (plate1 = 0, ...),
    ...
    # check if plate names contain spaces, then read them into the dictionary
...
def conv_dict(df_fromDict, i):
    """
    Converting one dictionary key into a variable because skbio
    does not like dataframes in dictionaries

    Args:
        df_fromDict (Dictionary):   The i'th index from the
        Dictionary created in ordinationbuild.filter_cols
        The key contains the according pd.DataFrame

        i (int): Iteration: Counts how many plates have been
        passed and processed. Max: Number of plates present.

    Returns:
        variabledf (pd.DataFrame): DataFrame containing the
        corresponding Dictionary Key according to i.
    """
...
def ordinationBuild(df2, i):
    ...
    Build Dataframe to be ready for processing through skbio.stats.ordination.
        Actions contain:
            ...
            4.  Converting well_id Chars into Numbers to be used for
                Coordination
    ...
    Args:
        ...
        i (int):    Iteration: Counts how many plates have been passed and
                    processed. Max: Number of plates present.
    Raises:
        KeyError:  Searching for "well_id" in Dataframe Column names.
                    Raises KeyError if "well_id" is not found or missing,
                    prompts User to create Column names "well_id"


    Returns:
        df3 (pd.DataFrame): Dataframe ready for ordinationbuilding through
                            skbio.stats.ordination. Only contains
                            "sample_name", "row" and "column" as other
    ...
    for val in df2["well_id"].values:
        ...
        if type(val) is not str:  # NaN is float
            df2 = df2.drop(x - 1, errors="ignore")
    ...
    # Important: A has to be counted as 8 and H as 1,
    # well_plates count from up -> down
    ...
    # converts all wellid values into numbers,
    # even nan values (they are still nan)
...
def finalDataframeBuild(finaldataframe):
    """
    Sets Type float for Values in "row" and "column".
    Might throw this one away ngl

    Args:
        finaldataframe (pd.DataFrame): dataframe from
                ordinationbuild.ordinationbuild

    Returns:
        samples (pd.DataFrame): Final Dataframe for
                                Ordination.
    """
...
def ordinationCreate(samples):
    ...
    Variables:
        eigvals (pd.Series):
            Made up Values. Its still necessary to give skbio
            some eigenvalues in order for the Ordination to build

        proportion_explained (pd:series):
            Made up Values. Its still necessary to give skbio some
            values in order for the Ordination to build

        Note:   I really cannot stress enough about how these Values are
                completely made up. If you want you can alter the Numbers
                to your personal liking, it doesn't matter at all, as long
                as only 2 Numbers are assigned to each eigenvals
                and proportion_explained.

    Args:
        samples (pd.DataFrame): DataFrame containing all the interesting
                                Infos for an Ordination

    Returns:
        ordination (skbio Ordinationfile):
            ordination. Can be converted into a .txt file with
...
def ordinationWrite(ordination, foldername, outputpath):
    ...
    Args:
        ordination (skbio Ordination):
        Ordination as a Result of ordinationbuild.ordinationCreate
        foldername (str):
        Foldername of Test/Study which contains the meta_plate.tsv file.
